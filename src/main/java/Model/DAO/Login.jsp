	...
	$(document).ready(function() {
		...
		$(document).ready(function() {
		    $('form[action="signup"]').submit(function(event) {
		        ...
		        var request1 = $.ajax({
		            method: "POST",
		            url: "http://localhost:8080/FastFood2/api/user/getUsername",
		            data: {username: username}
		        });

		        var request2 = $.ajax({
		            method: "POST",
		            url: "http://localhost:8080/FastFood2/api/user/getEmail",
		            data: {email: email}
		        });
		        ...
		        $.when(request1, request2).done(function(response1, response2) {
		            ...
		            if(!un.isSuccess) {
		            	...
		            	$.ajax({
		                    method: "POST",
		                    url: "http://localhost:8080/FastFood2/signup",
		                    data: $('form[action="signup"]').serialize(), 
		                    success: function(response) {
		                        alert("Đăng ký thành công!");
		                        window.location.href = 'http://localhost:8080/FastFood2/login'
		                    },
